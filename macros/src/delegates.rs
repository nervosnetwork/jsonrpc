...
use jsonrpc_pubsub::{self, PubSubMetadata, Subscriber, SubscriptionId};
...
impl<T, M, F> jsonrpc_pubsub::SubscribeRpcMethod<M> for DelegateSubscribe<T, F>
where
    M: PubSubMetadata,
...
impl<T, F, I> jsonrpc_pubsub::UnsubscribeRpcMethod for DelegateUnsubscribe<T, F>
where
    ...
    M: Metadata,
{
    ...
    pub fn add_alias(&mut self, from: &str, to: &str) {
        self.methods
            .insert(from.into(), RemoteProcedure::Alias(to.into()));
    }
    ...
    M: PubSubMetadata,
{
        ...
        I::Future: Send + 'static,
    {
        ...
        self.add_method_with_meta(unsubscribe.0, move |_, params, meta| {
            unsub.call(params, meta)
        });
    }
}
