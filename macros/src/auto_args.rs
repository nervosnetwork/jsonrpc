...
use crate::pubsub;
use crate::util::{expect_no_params, invalid_params, to_value};
...
use jsonrpc_core::{Error, Metadata, Params, Result, Value};
...
macro_rules! build_rpc_trait {
	...
	(
		...
		pub trait $name $(<$($generics,)*>)* : Sized + Send + Sync + 'static {
				...
				where $($($generics: Send + Sync + 'static + $crate::Serialize + $crate::DeserializeOwned),*)*
			{
				...
				$(
					$crate::build_rpc_trait!(WRAP del =>
						( $($t)* )
						fn $m_name ( $($p)* ) -> $result <$out $(, $error)* >
					);
				)*
	...
	( WRAP $del: expr =>
		...
		fn $method: ident (&self $(, $param: ident : $ty: ty)*) -> $result: tt <$out: ty $(, $error: ty)* >
	) => {
		$del.add_method($name, move |base, params| {
			$crate::WrapAsync::wrap_rpc(&(Self::$method as fn(&_ $(, $ty)*) -> $result <$out $(, $error)*>), base, params)
		});
		$(
			$(
	...
	( WRAP $del: expr =>
		...
		fn $method: ident (&self, $meta: ident : Self::Metadata $(, $param: ident : $ty: ty)*) -> $result: tt <$out: ty $(, $error: ty)* >
	) => {
		$del.add_method_with_meta($name, move |base, params, meta| {
			$crate::WrapMeta::wrap_rpc(&(Self::$method as fn(&_, Self::Metadata $(, $ty)*) -> $result <$out $(, $error)* >), base, params, meta)
		});
		$(
			$(
				$del.add_alias($alias, $name);
	...
	( WRAP $del: expr =>
		...
		fn $sub_method: ident (&self, $meta: ident : Self::Metadata $(, $sub_p: ident : $sub_ty: ty)+);
		unsubscribe: (name = $unsubscribe: expr $(, alias = [ $( $unsub_alias: expr, )+ ])*)
		fn $unsub_method: ident (&self $(, $unsub_p: ident : $unsub_ty: ty)+) -> $result: tt <$out: ty $(, $error_unsub: ty)* >;
	) => {
		$del.add_subscription(
			$name,
			($subscribe, move |base, params, meta, subscriber| {
				$crate::WrapSubscribe::wrap_rpc(
					&(Self::$sub_method as fn(&_, Self::Metadata $(, $sub_ty)*)),
					base,
					params,
					meta,
					subscriber,
			...
			($unsubscribe, move |base, id| {
				use jsonrpc_core::futures::{IntoFuture, Future};
...
type WrappedFuture<F, OUT, E> = future::MapErr<future::Map<F, fn(OUT) -> Value>, fn(E) -> Error>;
type WrapResult<F, OUT, E> = Either<WrappedFuture<F, OUT, E>, future::FutureResult<Value, Error>>;

fn as_future<F, OUT, E, I>(el: I) -> WrappedFuture<F, OUT, E>
where
    OUT: Serialize,
    E: Into<Error>,
...
impl<B, OUT, E, F, I> WrapAsync<B> for fn(&B) -> I
where
    B: Send + Sync + 'static,
...
impl<M, B, OUT, E, F, I> WrapMeta<B, M> for fn(&B, M) -> I
where
    M: Metadata,
    B: Send + Sync + 'static,
    OUT: Serialize + 'static,
...
impl<M, B, OUT> WrapSubscribe<B, M> for fn(&B, M, pubsub::Subscriber<OUT>)
where
    M: PubSubMetadata,
    B: Send + Sync + 'static,
    OUT: Serialize,
...
fn require_len(params: &Params, required: usize) -> Result<usize> {
    ...
    if len < required {
        return Err(invalid_params(
            &format!("`params` should have at least {} argument(s)", required),
            "",
        ));
    }
    Ok(len)
...
impl<B, OUT, T, E, F, I> WrapAsync<B> for fn(&B, Trailing<T>) -> I
where
    B: Send + Sync + 'static,
    OUT: Serialize + 'static,
    T: DeserializeOwned,
...
impl<M, B, OUT, T, E, F, I> WrapMeta<B, M> for fn(&B, M, Trailing<T>) -> I
where
    M: Metadata,
    B: Send + Sync + 'static,
    OUT: Serialize + 'static,
...
impl<M, B, OUT, T> WrapSubscribe<B, M> for fn(&B, M, pubsub::Subscriber<OUT>, Trailing<T>)
where
    M: PubSubMetadata,
    B: Send + Sync + 'static,
    OUT: Serialize,
    ...
    fn wrap_rpc(&self, base: &B, params: Params, meta: M, subscriber: Subscriber) {
        ...
        match id {
            Ok((id,)) => (self)(
                base,
                meta,
                pubsub::Subscriber::new(subscriber),
                Trailing(id),
            ),
            Err(e) => {
                let _ = subscriber.reject(e);
