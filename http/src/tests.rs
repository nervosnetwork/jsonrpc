...
fn should_return_method_not_allowed_for_get() {
    ...
    assert_eq!(
        response.status,
        "HTTP/1.1 405 Method Not Allowed".to_owned()
    );
    assert_eq!(
        response.body,
...
fn should_handle_health_endpoint_failure() {
    ...
    assert_eq!(
        response.status,
        "HTTP/1.1 503 Service Unavailable".to_owned()
    );
    assert_eq!(
        response.body,
...
fn should_return_unsupported_media_type_if_not_json() {
    ...
    assert_eq!(
        response.status,
        "HTTP/1.1 415 Unsupported Media Type".to_owned()
    );
    assert_eq!(
        response.body,
