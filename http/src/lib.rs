...
use crate::jsonrpc::futures::{self, Future};
...
pub struct ServerBuilder<
    M: jsonrpc::Metadata = (),
    S: jsonrpc::Middleware<M> = jsonrpc::middleware::Noop,
> {
...
impl<M: jsonrpc::Metadata, S: jsonrpc::Middleware<M>> ServerBuilder<M, S> {
    ...
    pub fn start_http(self, addr: &SocketAddr) -> io::Result<Server> {
        ...
        let (close, handle, local_addr_rx) = serve(
            "http.worker0".to_string(),
            addr.to_owned(),
            cors_domains.clone(),
            cors_max_age,
        ...
        let mut handles = handles
            .into_iter()
            .map(|(close, handle, local_addr_rx)| {
                let _ = recv_address(local_addr_rx)?;
                Ok((close, handle))
            })
            .collect::<io::Result<(Vec<_>)>>()?;
        handles.push((close, handle));
        let (close, handles) = handles.into_iter().unzip();
...
fn serve<M: jsonrpc::Metadata, S: jsonrpc::Middleware<M>>(
    ...
    oneshot::Sender<()>,
    thread::JoinHandle<()>,
    mpsc::Receiver<io::Result<SocketAddr>>,
) {
    ...
    let thread = thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let graceful = bind_result.and_then(move |(listener, local_addr)| {
                let allowed_hosts = server_utils::hosts::update(allowed_hosts, &local_addr);
                let builder = server::Server::from_tcp(listener).unwrap();
                let service = move || {
                    ServerHandler::new(
                        jsonrpc_handler.clone(),
                        cors_domains.clone(),
                ...
                builder.http1_keepalive(keep_alive)
                    .serve(service)
                    .with_graceful_shutdown(shutdown_signal)
                    .map_err(|err| error!("server error: {}", err))
            });
            let _ = tokio::runtime::current_thread::block_on_all(graceful);
        })
        .expect("Couldn't spawn a thread.");

    (close, thread, local_addr_rx)
}
...
impl Server {
    ...
    pub fn close(mut self) {
        ...
        for handle in self.handles.take().expect(PROOF) {
            let _ = handle.join();
        }
    }
}
